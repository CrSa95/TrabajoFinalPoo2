...
public class ReporteAduana implements Reporte {
	...
	public String emitir(Buque buque) {
		...
		html.abrir("html").abrir("body").elemento("p", "Nombre del buque: " + buque.nombre())
				.elemento("p", "Fecha de arribo: " + viaje.fechaLlegada(terminal))
				.elemento("p", "Fecha de partida: " + viaje.getFechaSalida()).abrir("ul");

		terminal.cargasEnViaje(viaje).stream().forEach(carga -> html.elemento("li", carga.tipo() + " | " + carga.id()));
